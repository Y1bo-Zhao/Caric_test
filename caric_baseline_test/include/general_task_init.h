...
class gcs_task_assign{
    public:
        ...
        : nh_ptr_(nh_ptr_)//相当于在构造函数中多一行nh_ptr = nh_ptr
        {
            ...
            box0_pub_ = nh_ptr_->advertise<std_msgs::String>("/box/jurong", 10);
            box1_pub_ = nh_ptr_->advertise<std_msgs::String>("/box/raffles", 10);
            box0_index_pub_ = nh_ptr_->advertise<std_msgs::String>("/start/jurong", 10);//发送起始点
            box1_index_pub_ = nh_ptr_->advertise<std_msgs::String>("/start/raffles", 10);

            back_sub_0 = nh_ptr_->subscribe<std_msgs::Int32>("/jurong/back", 1, &gcs_task_assign::backCallback_0, this);
            back_sub_1 = nh_ptr_->subscribe<std_msgs::Int32>("/raffles/back", 1, &gcs_task_assign::backCallback_1, this);
        }
    private:
        ...
        ros::Publisher box0_pub_;
        ros::Publisher box1_pub_;
        ros::Publisher box0_index_pub_;
        ros::Publisher box1_index_pub_;



        ros::Subscriber agent_position_sub_;
        ros::Subscriber bbox_sub_;
        ros::Subscriber back_sub_0;
        ros::Subscriber back_sub_1;
        ...
        Eigen::Vector3d next_point_0;
        Eigen::Vector3d next2_point_0;
        int point_index_0=0;
        ...
        vector<vector<Eigen::Vector3d>> points_for_explorers;
        ...
        void backCallback_0(const std_msgs::Int32 msg)
        {
            point_back_0 = msg.data;
            cout <<"back" <<point_back_0<<endl;
        }

        void backCallback_1(const std_msgs::Int32 msg)
        {
            point_back_1 = msg.data;
        }
        ...
        void bboxCallback(const sensor_msgs::PointCloud::ConstPtr &msg) //一次性函数
        {
            ...
            if (num_points % 8 == 0 && num_points > 8 * box_set.size())//box_set一开始为0，新来的点的数量可以构成box且后半条件不懂，难道不是一次全读进来？
            {
                ...
                for (int i = 0; i < num_box; i++)
                {
                    ...
                    z_limit = (zmax+zmin)/2; //explorer上下分两半扫描
                    std::cout << "The mid_z of is: " << z_limit << "zmax:" <<zmax << "zmin:" << zmin <<std::endl;
                ...
                //----------------------------------------------------------------------3刀流----------------------------
                /*
                points_for_explorers.resize(4);
                for (int i = 0; i < 3; i++)
                {
                    vector<Eigen::Vector3d> points_cut(100);
                    double z_cut = 0.25*(i+1) * (zmax-zmin);
                    for (int j = 0; j < size; j++)
                    {
                        if(z_cut > box_set[j].getVertices()[0].z() && z_cut < box_set[j].getVertices()[4].z())
                        {
                            points_cut.push_back(Eigen::Vector3d(box_set[j].getVertices()[0].x(),box_set[j].getVertices()[0].y(),z_cut));
                            points_cut.push_back(Eigen::Vector3d(box_set[j].getVertices()[1].x(),box_set[j].getVertices()[1].y(),z_cut));
                            points_cut.push_back(Eigen::Vector3d(box_set[j].getVertices()[2].x(),box_set[j].getVertices()[2].y(),z_cut));
                            points_cut.push_back(Eigen::Vector3d(box_set[j].getVertices()[3].x(),box_set[j].getVertices()[3].y(),z_cut));
                            
                        }

                    }
                    cout <<"xiba" << endl;
                    points_for_explorers[i] = convexHull(points_cut,points_cut.size());
                    cout <<"xiba2" << endl;
                    
                }
                cout << "三刀流："<< points_for_explorers.size() << endl;
                */
                ...
                for(int i=0;i<size;i++)
                {
                    ...
                    low = vertice_temp[0];                
                    hign = vertice_temp[4];
                    std::cout << "hign:" << hign.z() << "low:" << low.z() <<std::endl;
                    mid = Eigen::Vector3d(low.x(), low.y(), z_limit);
                    ...
                    {            
                        ...
                        vector<Eigen::Vector3d> new_vertice_0;
                        vector<Eigen::Vector3d> new_vertice_1;
                        new_vertice_0.resize(8);
                        new_vertice_1.resize(8);
                ...
                //Team_allocate();
                Get_search_order();
                std::cout << "The num of box_0 is: " << box_set_0.size() << std::endl;
                std::cout << "The num of box_1 is: " << box_set_1.size() << std::endl;
    ...
    void Get_search_order(){//找最近的box顶点作为起点
        ...
        //std::cout << "The num of box_0 is: " << box_index_0.size() << std::endl;
        ...
        std_msgs::String task_0_init;
        task_0_init.data=to_string(start_point_0.x())+","+to_string(start_point_0.y())+","+to_string(start_point_0.z());
        box0_index_pub_.publish(task_0_init);

        std_msgs::String task_1_init;
        task_1_init.data=to_string(start_point_1.x())+","+to_string(start_point_1.y())+","+to_string(start_point_1.z());
        box1_index_pub_.publish(task_1_init);
    ...
    {   
        if(finish_box_order_generate)
        {
            ...
            task.data=to_string(target_point_0.x())+","+to_string(target_point_0.y())+","+to_string(target_point_0.z())+";"+ to_string(next_point_0.x())+","+to_string(next_point_0.y())+","+to_string(next_point_0.z())+";" + to_string(next2_point_0.x())+","+to_string(next2_point_0.y())+","+to_string(next2_point_0.z())+";" +to_string(target_point_1.x())+","+to_string(target_point_1.y())+","+to_string(target_point_1.z())+";"+to_string(next_point_1.x())+","+to_string(next_point_1.y())+","+to_string(next_point_1.z());
            //task.data=
    ...
    void Get_next_point_to_go_0()
    {
        ...
        //cout<<"axiba1"<<endl;
        point_temp_0 = box_set_0[box_index_0[target_box_0]].getVertices()[4];
        Eigen::Vector3d point_temp_0_1 = box_set_0[box_index_0[target_box_0]].getVertices()[0];
        double height = point_temp_0(2) - point_temp_0_1(2);
        Eigen::Vector3d goal_0 = Eigen::Vector3d(point_temp_0_1(0),point_temp_0_1(1),0.75*height+point_temp_0_1(2));
        
        //cout<<"飞机当前高度～"<< position_pair["/jurong"].position.z() <<endl;
        //cout<<"当前盒子最高～"<< point_temp_0.z() <<endl;
        //if(  (goal_0-position_pair["/jurong"].position).norm()<6 )//一个box扫描完毕
        if(point_back_0 == 10)
        {
            ...
            if(target_box_0+1<box_index_0.size())//~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            {
                ...
                height_index_0 = 0;
                point_index_0 = 0;
                cout<<"jurong换盒："<<box_index_0[target_box_0]<<endl;
                point_back_0 = 0;
            }
            else
            {
        ...
        //cout<<"此时的盒子："<<box_index_0[target_box_0]<<endl;
        point_0.resize(5);
        /*point_0[0] = Eigen::Vector3d(xmin, ymin, zmin);
        point_0[1] = Eigen::Vector3d(xmax, ymin, zmin);
        point_0[2] = Eigen::Vector3d(xmax, ymax, zmin);
        point_0[3] = Eigen::Vector3d(xmin, ymax, zmin);
        point_0[4] = Eigen::Vector3d(xmin, ymin, z_limit);*/
        ...
        std_msgs::String task_0;
        task_0.data=to_string(point_0[0].x())+","+to_string(point_0[0].y())+","+to_string(point_0[0].z())+";"+ \
                    to_string(point_0[1].x())+","+to_string(point_0[1].y())+","+to_string(point_0[1].z())+";"+ \
                    to_string(point_0[2].x())+","+to_string(point_0[2].y())+","+to_string(point_0[2].z())+";"+ \
                    to_string(point_0[3].x())+","+to_string(point_0[3].y())+","+to_string(point_0[3].z())+";"+ \
                    to_string(point_0[4].x())+","+to_string(point_0[4].y())+","+to_string(point_0[4].z());
        box0_pub_.publish(task_0);
                
        /*cout << "p1:"<<point_0[0] <<endl;
        cout << "p2:"<<point_0[1] <<endl;
        cout << "p3:"<<point_0[2] <<endl;
        cout << "p4:"<<point_0[3] <<endl;
        cout << "p5:"<<point_0[4] <<endl;*/

        
        //cout<<"axiba3"<<endl;
        //cout <<"now positon-0:"<<position_pair["/jurong"].position<<endl;
        //cout <<"target-0:"<<target_point_0<<endl;
        //cout <<(position_pair["/jurong"].position - target_point_0).norm()<<endl;
        if( (position_pair["/jurong"].position - target_point_0).norm()<0.1 && inout==false )//飞机到达上一目标后
        {
            ...
            next_point_0.x() = point_0[(point_index_0+1)-((point_index_0+1)/4)*4](0);
            next_point_0.y() = point_0[(point_index_0+1)-((point_index_0+1)/4)*4](1);
            next_point_0.z() = point_0[(point_index_0+1)-((point_index_0+1)/4)*4](2)+10+5*((height_index_0+1)/4);
            next2_point_0.x() = point_0[(point_index_0+1)-((point_index_0+1)/4)*4](0);
            next2_point_0.y() = point_0[(point_index_0+1)-((point_index_0+1)/4)*4](1);
            next2_point_0.z() = point_0[(point_index_0+1)-((point_index_0+1)/4)*4](2)+10+5*((height_index_0+1)/4);

            inout = true;
        }
        if( (position_pair["/jurong"].position - target_point_0).norm()>0.1 && inout==true )
        {
            inout = false;
        }
    ...
    void Get_next_point_to_go_1()
    {
        ...
        //cout<<"axiba1"<<endl;
        point_temp_1 = box_set_1[box_index_1[target_box_1]].getVertices()[4];
        Eigen::Vector3d point_temp_1_1 = box_set_1[box_index_1[target_box_1]].getVertices()[0];
        double height = point_temp_1(2) - point_temp_1_1(2);
        Eigen::Vector3d goal_1 = Eigen::Vector3d(point_temp_1_1(0),point_temp_1_1(1),0.75*height+point_temp_1_1(2));
        //if(  (position_pair["/raffles"].position -goal_1).norm()<6  )//一个box扫描完毕
        if(point_back_1 == 10)
        {
            if(target_box_1+1<box_index_1.size())
            {
                ...
                height_index_1 = 0;
                point_index_1 = 0;
                cout<<"raffles换盒："<<box_index_1[target_box_1]<<endl;
                point_back_1 = 0;
        ...
        std_msgs::String task_1;
        task_1.data=to_string(point_1[0].x())+","+to_string(point_1[0].y())+","+to_string(point_1[0].z())+";"+ \
                    to_string(point_1[1].x())+","+to_string(point_1[1].y())+","+to_string(point_1[1].z())+";"+ \
                    to_string(point_1[2].x())+","+to_string(point_1[2].y())+","+to_string(point_1[2].z())+";"+ \
                    to_string(point_1[3].x())+","+to_string(point_1[3].y())+","+to_string(point_1[3].z())+";"+ \
                    to_string(point_1[4].x())+","+to_string(point_1[4].y())+","+to_string(point_1[4].z());
        box1_pub_.publish(task_1);

        
        
        //cout <<"now positon—1:"<<position_pair["/raffles"].position<<endl;
        //cout <<"target-1:"<<target_point_1<<endl;
        //cout <<(position_pair["/raffles"].position - target_point_0).norm()<<endl;
        if(  (position_pair["/raffles"].position - target_point_1).norm()<0.5    )//飞机到达上一目标后
        {
            
            target_point_1.x() = point_1[point_index_1-(point_index_1/4)*4](0);
            target_point_1.y() = point_1[point_index_1-(point_index_1/4)*4](1);
            target_point_1.z() = point_1[point_index_1-(point_index_1/4)*4](2)+4+5*(height_index_1/4);
            point_index_1++;
            height_index_1++;
            next_point_1.x() = point_1[point_index_1-(point_index_1/4)*4](0);
            next_point_1.y() = point_1[point_index_1-(point_index_1/4)*4](1);
            next_point_1.z() = point_1[point_index_1-(point_index_1/4)*4](2)+4+5*(height_index_1/4);
            outin = true;

                //cout << "1_换点" << target_point_1 <<endl;
            
        }
        if( (position_pair["/raffles"].position - target_point_1).norm()>0.5 && outin==true )
        {
            outin = false;
        }
        //position_pair["raffles"].update=false;
    }
