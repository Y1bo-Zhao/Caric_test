...
class gcs_task_assign{
    ...
    void Get_next_point_to_go_0()
    {
        ...
        if(  point_temp_0.z()-position_pair["/jurong"].position.z()<2 )//一个box扫描完毕
        {
            //cout<<"axiba2"<<endl;
        ...
        if( (position_pair["/jurong"].position - target_point_0).norm()<2  )//飞机到达上一目标后
        {
            if(point_index_0>=4)
            {
                height_index_0++;
    ...
    void Get_next_point_to_go_1()
    {
        ...
        if(  std::abs(position_pair["/raffles"].position.z() - point_temp_1.z())<2  )//一个box扫描完毕
        {
        ...
        if(  (position_pair["/raffles"].position - target_point_1).norm()<2    )//飞机到达上一目标后
        {
            if(point_index_1>=4)
            {
                height_index_1++;
