...
public:
    ...
    : nh_ptr(nh_ptr_)
    {
        ...
        box_index_sub_   = nh_ptr->subscribe("/box_index", 10, &Agent::boxindexCallback, this)
...
private:
    ...
    */
    ...
    void boxCallback(vector<Boundingbox> box_temp)
    {
        box_set = box_temp;
    ...
    void boxindexCallback(vector<int> box_index_temp)
    {
        box_index = box_index_temp;
    }
    ...
    void Get_next_point_to_go()
    {
        ...
        //cout<<"axiba1"<<endl;
        point_temp = box_set[box_index[target_box]].getVertices()[7];
        //cout<<"飞机当前高度～"<< position_pair["/jurong"].position.z() <<endl;
        //cout<<"当前盒子最高～"<< point_temp_0.z() <<endl;
        if(  point_temp.z()-position_pair["/jurong"].position.z()<0.5 )//一个box扫描完毕
        {
            cout<<"换盒"<<endl;
