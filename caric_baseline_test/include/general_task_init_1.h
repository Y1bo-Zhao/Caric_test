...
class gcs_task_assign{
    public:
        ...
        : nh_ptr_(nh_ptr_)//相当于在构造函数中多一行nh_ptr = nh_ptr
        {
            ...
            box_pub_0 = nh_ptr_->advertise<vector<Boundingbox>>("/box_0", 10);
            box_pub_1 = nh_ptr_->advertise<vector<Boundingbox>>("/box_1", 10);
            box_index_pub_0 = nh_ptr_->advertise<vector<int>>("/box_index_0", 10);
            box_index_pub_1 = nh_ptr_->advertise<vector<int>>("/box_index_1", 10);
            //边界框的顶点发布在此topic下
    ...
    private:
        ...
        ros::Publisher box_pub_0;
        ros::Publisher box_pub_1;
        ros::Publisher box_index_pub_0;
        ros::Publisher box_index_pub_1;
    ...
    {   
        ...
        {
            box_index_pub_0.publish(box_index_0);
            box_index_pub_1.publish(box_index_1);
            box_pub_0.publish(box_set_0);
            box_pub_1.publish(box_set_1);
    ...
    void Get_next_point_to_go_0()
    {
        ...
        if(  point_temp_0.z()-position_pair["/jurong"].position.z()<0.5 )//一个box扫描完毕
        {
            cout<<"换盒"<<endl;
        ...
        if( (position_pair["/jurong"].position - target_point_0).norm()<0.5 && inout==false )//飞机到达上一目标后
        {
            ...
            target_point_0.z() = point_0[point_index_0-(point_index_0/4)*4](2)+10+5*height_index_0;
            point_index_0++;
            height_index_0++;
            inout = true;
        }
        if( (position_pair["/jurong"].position - target_point_0).norm()>0.5 && inout==true )
        {
            inout = false;
        }
    ...
    void Get_next_point_to_go_1()
    {
        ...
        if(  (position_pair["/raffles"].position - target_point_1).norm()<0.5    )//飞机到达上一目标后
        {
            if(point_index_1<=3)
            {
                ...
                target_point_1.z() = point_1[point_index_1](2)+5;
                point_index_1++;
            ...
            if(point_index_1>3)
            {
                ...
                target_point_1.z() = point_1[point_index_1-(point_index_1/4)*4](2)+5+2*height_index_1;
                point_index_1++;
                height_index_1++;
                //cout << "1_换点" << target_point_1 <<endl;
            }
