...
class gcs_task_assign{
    ...
    private:
        ros::NodeHandlePtr nh_ptr_;gcs_task_assign
        ...
        void bboxCallback(const sensor_msgs::PointCloud::ConstPtr &msg) //一次性函数
        {
            ...
            if (num_points % 8 == 0 && num_points > 8 * box_set.size())//box_set一开始为0，新来的点的数量可以构成box且后半条件不懂，难道不是一次全读进来？
            {
                ...
                for(int i=0;i<size;i++)
                {
                    ...
                    if(hign>z_limit&&low<z_limit)//判断该box是否需要中分
                    {            
                    ...
                    else if(hign<=z_limit) //下半区
                    {
                        for(int j=0;j<8;j++)
                        {
                            point_vec_0.push_back(vertice_temp[j]);
                        }
                        box_set_0.push_back(Boundingbox(point_vec_0, plus));                       
                        plus_0++;
                        point_vec_0.clear(); 
                        point_vec_1.clear(); 
                    }
                    else if(low>=z_limit)  //上半区
                    {
                        for(int j=0;j<8;j++)
                        {
                            point_vec_1.push_back(vertice_temp[j]);
                        }
                        box_set_1.push_back(Boundingbox(point_vec_1, plus));
                        plus_1++;
                        point_vec_0.clear(); 
                        point_vec_1.clear(); 
                    }
    ...
    void Get_search_order(){//找最近的box顶点作为起点
        ...
        while(box_index_0.size()<box_set_0.size()){
            ...
            for(int i=0;i<box_set_0.size();i++){
                if(find(box_index_0.begin(),box_index_0.end(),i)！=box_index_0.end()&&box_index_0.size()>0){//find作用于容器就是这个用法，!=意思是找到了，第一次之后第二个条件就满足了
                    continue;
                }
                for(int j=0;j<8;j++){  
    ...
    void generate_next_position()//把两个explorer的下一个目标位置发出去,加一个bool变量
    {
        for()
        result="";
    ...
    void Get_next_point_to_go_0()
    {
        ...
        Eigen::Vector3d point_temp = box_set_0[box_index_0[target_box_0]].getVertices()[7];
        if(position_pair["/jurong"].position(2) == point_temp(2))//一个box扫描完毕
        {
            if(target_box_0<=box_set_0.size())
            {
        ...
        if(position_pair["/jurong"].position == target_point_0)//飞机到达上一目标后
        {
            if(point_index_0>=4)
            {
                height_index++;
                point_index_0 = 0;
                target_point_0(0) = point_0[point_index_0](0);
                target_point_0(1) = point_0[point_index_0](1);
                target_point_0(2) = point_0[point_index_0](2)+2*height_index;  
                point_index_0++;
            }
            if(point_index_0<=3)
            {
                target_point_0(0) = point_0[point_index_0](0);
                target_point_0(1) = point_0[point_index_0](1);
                target_point_0(2) = point_0[point_index_0](2);
                point_index_0++;
            }
