...
class gcs_task_assign{
    ...
    private:
        ...
        void bboxCallback(const sensor_msgs::PointCloud::ConstPtr &msg) //一次性函数
        {
            ...
            sensor_msgs::PointCloud cloud = *msg;  //sensor_msgs::PointCloud.points
            int num_points = cloud.points.size();  //vector.size()
            if (num_points % 8 == 0 && num_points > 8 * box_set.size())//box_set不是=0？
            {
                volumn_total = 0;
        ...
        void Team_allocate()
        {
            ...
            if(position_pair["/jurong"].update){    //0 = jurong 
                team_info[0].push_back("/jurong");
            }
            if(position_pair["/raffles"].update){  //1 = raffles
    ...
    void Best_first_search(){ //找出发点 我们要先让一个升空后再用两次这个函数的变形
        ...
        while(box_index.size()<box_set.size()){
            ...
            for(int i=0;i<box_set.size();i++){
                if(find(box_index.begin(),box_index.end(),i)!=box_index.end()&&box_index.size()>0){ //在已经找到过的最小距离盒子中没有该数字或找到的数字不是最后一个 且不是第一次//！=换成==？
                    continue;
                }
                for(int j=0;j<2;j++){  
