...
public:
    ...
    void update_map(const sensor_msgs::PointCloud2ConstPtr &cloud, const sensor_msgs::PointCloud2ConstPtr &Nbr, const nav_msgs::OdometryConstPtr &msg)
    {
        ...
        for (const auto &point : cloud_cloud->points)
        {
            ...
            if (is_Nbr(cloud_cloud_point, Nbr_point))//检验是否是其他UAV被扫到时生成的点云
            {
                continue;
            }
            else
    ...
    // communication part
    bool get_position_plan_msg(string &msg) //在有里程计数据且完成初始化的情况下返回false，在is_planned或其他情况下返回ture，并把现在位置以及路径点传到msg里
    {
        if (!odom_get || !finish_init) 
        {
            return false;
...
public:
    ...
    : nh_ptr(nh_ptr_)
    {
        ...
        //--------------------------------------------------------------------------------------------------------------------------
        //firefly在launch文件中被map成agent的名字
                     ...
                     const nav_msgs::OdometryConstPtr &msg)
    {
        ...
        mm.update_map(cloud, Nbr, msg);//用点云、其他UAV和自己的位置来更新地图
    }
...
private:
    ...
    void TimerProbeNbrCB(const ros::TimerEvent &) //前提：服务可用且地图完成初始化
    {
        ...
        std_msgs::String msg;
        if (mm.get_position_plan_msg(msg.data))//msg.data的类型是string，在有里程计数据且完成初始化的情况下返回false，在is_planned或其他情况下返回ture，并把现在位置以及路径点传到msg里
        {
            communication_pub_.publish(msg);//把发到broadcast上
        }
        else
        {
            return;
    ...
    void TimerPlanCB(const ros::TimerEvent &)//前提：地图初始化
    {
        if (!map_initialise)
        {
            return;
