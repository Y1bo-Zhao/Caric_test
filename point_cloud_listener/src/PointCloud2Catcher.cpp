...
// 创建一个PointCloud<pcl::PointNormal>对象
    ...
    for (size_t i = 0; i < all_clouds->points.size(); ++i)
    {
        ...
        for(int j=0; j<boxes.size(); j++)
        {
                ...
                point.z >= boxes[j].getCenter().z() - 0.5 * boxes[j].getZsize() && point.z <= boxes[j].getCenter().z() + 0.5 * boxes[j].getZsize())
            {
                ...
                // 修正normal的方向
                if (
                      (point.x - boxes[j].getCenter().x()) * normal.normal_x +
                      (point.y - boxes[j].getCenter().y()) * normal.normal_y +
                      (point.z - boxes[j].getCenter().z()) * normal.normal_z > 0
                   )
                {
                ...
                {
                      pcl::Normal flipped_normal;
                      flipped_normal.normal_x = -normal.normal_x;
                      flipped_normal.normal_y = -normal.normal_y;
                      flipped_normal.normal_z = -normal.normal_z;
                      objectsMap[j].push_back(PointCloudObject(point, j, flipped_normal));
                }
...
void VectorOutput(visualization_msgs::MarkerArray& marker_array)
{
    ...
    {   
        for (const auto& object : pair.second)
        {
            ...
            end.x = start.x + object.getNormal().normal_x ; 
            end.y = start.y + object.getNormal().normal_y ; 
            end.z = start.z + object.getNormal().normal_z ;
...
void TargetPointOutput(sensor_msgs::PointCloud2& target_point_cloud)
{

    // 设置点云的字段
    pcl::PointCloud<pcl::PointXYZ> pcl_tar_cloud;
    // 填充消息头部
    pcl_tar_cloud.header.stamp = ros::Time::now().toNSec();
    pcl_tar_cloud.header.frame_id = "world";
    ...
    {   
        ...
        {
            pcl::PointXYZ point;
            point.x = object.getPoint().x ;
            point.y = object.getPoint().y ;
            point.z = object.getPoint().z ;
            pcl_tar_cloud.points.push_back(point);
        }
    }

    // 转换PCL点云到sensor_msgs::PointCloud2
    pcl::toROSMsg(pcl_tar_cloud, target_point_cloud);
}

void PositionPointOutput(sensor_msgs::PointCloud2& position_point_cloud)
{
    int scale_normal_length = 3; //这个变量决定了法向量长度，即你的无人机应该飞到距离目标点多远的位置，可改

    // 设置点云的字段
    pcl::PointCloud<pcl::PointXYZ> pcl_cloud;
    pcl_cloud.header.stamp = ros::Time::now().toNSec();
    ...
    {   
        for (const auto& object : pair.second)
        {
            ...
            pcl::PointXYZ point;
            point.x = object.getPoint().x + object.getNormal().normal_x * scale_normal_length ;
            point.y = object.getPoint().y + object.getNormal().normal_y * scale_normal_length ;
            point.z = object.getPoint().z + object.getNormal().normal_z * scale_normal_length ;
            pcl_cloud.points.push_back(point);
        }
    }

    // 转换PCL点云到sensor_msgs::PointCloud2
    pcl::toROSMsg(pcl_cloud, position_point_cloud);
}

// 创建一个MarkerArray消息
visualization_msgs::MarkerArray marker_array;
// 创建一个PC2消息
sensor_msgs::PointCloud2 target_point_cloud;
// 创建一个PC2消息
sensor_msgs::PointCloud2 position_point_cloud;
...
/*     // 创建一个Publisher发布点云消息
    ...
    // 创建一个Publisher，发布计算出的向量消息（可视化）
    ros::Publisher marker_pub = nh.advertise<visualization_msgs::MarkerArray>("normal_marker_array", 1);
    // 创建一个Publisher，发布要扫描的目标点位置。
    ros::Publisher target_point_pub = nh.advertise<sensor_msgs::PointCloud2>("target_point_cloud", 1);
    // 创建一个Publisher，发布无人机的路径点位置。
    ros::Publisher position_point_pub = nh.advertise<sensor_msgs::PointCloud2>("position_point_cloud", 1);
    ...
    while (ros::ok()) {
        ...
        VectorOutput(marker_array);
        TargetPointOutput(target_point_cloud);
        PositionPointOutput(position_point_cloud);

        marker_pub.publish(marker_array);//发布向量消息
        target_point_pub.publish(target_point_cloud);//发布目标点点云消息
        position_point_pub.publish(position_point_cloud);//发布位置点点云消息
        ros::spinOnce();
        // 按照设定的频率休眠
        loop_rate.sleep();
    }
    ...
    // 将点云数据和法向量数据复制到cloud_with_normals
    //pcl::concatenateFields(*all_clouds, *cloud_normals, *cloud_with_normals);
    ...
    //std::cout << "Saved " << cloud_normals->points.size() << " data points to normals.pcd." << std::endl;
    //std::cout << "Saved " << cloud_with_normals->points.size() << " data points to cloud_with_normals.pcd." << std::endl;
